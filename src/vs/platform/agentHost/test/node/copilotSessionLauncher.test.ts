...
suite('CopilotSessionLauncher resume config', () => {
	...
	function buildResumeConfig(
	...
	): Promise<{ model?: string; reasoningEffort?: string; contextTier?: string; availableTools?: string[]; excludedTools?: string[]; modelCapabilities?: Record<string, unknown>; toolSearch?: { enabled: boolean }; enableExperimentalMode?: boolean; enableSessionStore?: boolean }> {
		const plan = {
			kind: 'resume',
		...
		return (launcher as unknown as { _buildSessionConfig(plan: unknown, runtime: unknown, onManagedSettingsResolved: () => void): Promise<{ model?: string; reasoningEffort?: string; contextTier?: string; availableTools?: string[]; excludedTools?: string[]; modelCapabilities?: Record<string, unknown>; toolSearch?: { enabled: boolean }; enableExperimentalMode?: boolean; enableSessionStore?: boolean }> })._buildSessionConfig(plan, runtime, () => { });
	}
	...
	test('disables the CLI session store so list-sessions follow-ups hit the host tool (#336647)', async () => {
		const store = new DisposableStore();
		const config = await buildResumeConfig(createLauncher(store, {}), { id: 'claude-sonnet-4.6' });
