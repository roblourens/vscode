...
|---|---|---|
| `Steering` | Inject *into the running turn* as additional context. The model sees it before its current generation completes. | Set while turn is in flight; promoted to its own turn on the intermediate `result`; `steering_consumed` is the abort/dispose fallback. |
...
steer aborted. (The SDK does not echo a steering `SDKUserMessage`
on the event stream.) Either way the pill clears when the model has
*seen* the steering, not when the queue accepted it.
...
- **Steering promotes to its own Turn at the protocol level when the
  SDK preempts internally.** On the Claude SDK, `priority: 'now'`
  causes the SDK to abort the current generation and run the
  steering message next. The `result` for that aborted generation
  ends the interrupted Turn and opens the steering Turn, so
  consumers see the steer as its own row rather than as silent
  context for the previous one. Provider implementations
  must yield via `priority: 'now'` (or the SDK's equivalent
  preempt hint), **not** via `Query.interrupt()` followed by a new
  send — that path produces explicit Turn boundaries without the
  steering content.
...
- **Steering creates a new `Turn.id` at the intermediate `result`.**
  The promoted turn reuses the pending-message id (which is also the
  SDK uuid of the steer). UIs that key off Turn boundaries see the
  steer as a separate row.
- **`steering_consumed` is the fallback for a steer that never ran.**
  The pending pill normally clears via `ChatTurnStarted.queuedMessageId`
  when the steering turn starts. `steering_consumed` fires on abort
  or dispose so a steer the SDK accepted but never ran does not leave
  a stuck bubble.
