...
| `disallowedTools` | `['WebSearch']` (proxy only) | CAPI doesn't support WebSearch; omit on native Anthropic-auth sessions where the tool works. | `claudeSdkOptions.ts` |
...
- **`executable: process.execPath as 'node'`** — **required** (`claudeCodeAgent.ts:437`).
  ...
  the utility process.
- **`disallowedTools: ['WebSearch']`** — **required on proxy** (`claudeSdkOptions.ts`).
  CAPI does not yet support the WebSearch tool. Without this the SDK may
  invoke it and the upstream CAPI call will fail mid-session. Native
  Anthropic-auth sessions omit this so WebSearch stays available.
