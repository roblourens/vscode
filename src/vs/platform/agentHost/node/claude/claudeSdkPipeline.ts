 ...
 *     deferred on `result`, and emit `ChatTurnComplete` when the
 *     queue fully drains. An intermediate result that preempts for
 *     steering completes the interrupted turn and starts a steering
 *     turn (CONTEXT.md M10).
 *
...
export class ClaudeSdkPipeline extends Disposable {
	...
	private _beginSteeringTurn(steering: PendingMessage, interrupted: IPendingSdkMessage): void {
		...
		// Reuse the pending-message id as the turn id: Claude already sends the
		// steer under that id as the SDK uuid (`Turn.id === SDK uuid`).
		...
		try {
			for await (const message of query) {
				...
				if (message.type === 'result') {
					...
					// Intermediate result with a pending steer promotes that
					// message to its own turn (CONTEXT.md M10).
